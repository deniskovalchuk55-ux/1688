...
import { sourceProducts, refreshMarket } from './sourceProducts.js';
import { groupTrends } from './clustering.js';
import { expandBroad } from './expand.js';
  ...
  try {
    ...
    const rows = await query(`
      SELECT
        ...
        (SELECT level FROM trend_market WHERE trend_id = t.id) AS competition
      FROM trends t
...
app.get('/api/trends/:id', async (req, res) => {
  try {
    ...
    const [market] = await query(
      'SELECT * FROM trend_market WHERE trend_id = $1', [id]);
...
// Список товарів — те, що бачиш на головній.
app.get('/api/groups', async (req, res) => {
  try {
    const { category, age, limit = 25 } = req.query;
    const where = ['g.broad = false'];
    const params = [];

    if (category) { params.push(category); where.push(`g.category = $${params.length}`); }
    if (age) { params.push(age); where.push(`g.age_group = $${params.length}`); }
    params.push(Math.min(Number(limit) || 25, 100));

    const rows = await query(`
      SELECT
        g.id, g.name, g.category, g.age_group,
        s.momentum, s.volume, s.confidence, s.score, s.members,
        m.market_avg, m.level AS competition, m.sellers, m.demand,
        (SELECT COUNT(*)::int FROM products WHERE group_id = g.id) AS product_count,
        (SELECT MAX((profit->'best'->>'profitUah')::numeric)
           FROM products WHERE group_id = g.id) AS top_profit,
        (SELECT MAX(growth_pct) FROM trend_signals sg
           JOIN trends t ON t.id = sg.trend_id
          WHERE t.group_id = g.id AND sg.source = 'google_trends'
            AND sg.captured_at > now() - interval '2 days') AS growth_pct
      FROM trend_groups g
...
app.get('/api/groups/:id', async (req, res) => {
  try {
    ...
    const [group] = await query(
      `SELECT g.*, s.momentum, s.volume, s.confidence, s.score, s.members
       FROM trend_groups g
       LEFT JOIN trend_group_scores s ON s.group_id = g.id
       WHERE g.id = $1`, [id]);

    if (!group) return res.status(404).json({ error: 'Товар не знайдено' });

    // Усі формулювання, за якими люди шукають цей товар.
    const members = await query(
      `SELECT t.id, t.title, sc.score,
        (SELECT growth_pct FROM trend_signals
          WHERE trend_id = t.id AND source = 'google_trends'
          ORDER BY captured_at DESC LIMIT 1) AS growth_pct
       FROM trends t
       JOIN trend_scores sc ON sc.trend_id = t.id
       WHERE t.group_id = $1 AND t.status = 'active'
       ORDER BY sc.score DESC`, [id]);
    ...
    const [market] = await query(
      'SELECT * FROM trend_market WHERE group_id = $1', [id]);

    const [q] = await query(
      'SELECT query_cn, note FROM trend_queries WHERE group_id = $1', [id]);

    const media = await query(
      `SELECT m.* FROM trend_media m
       JOIN trends t ON t.id = m.trend_id
       WHERE t.group_id = $1 ORDER BY m.views DESC LIMIT 4`, [id]);
...
async function runCollect(reason) {
  ...
  try {
    ...
    // Зводимо різні формулювання в товари. Раз на добу досить.
    const [lastGroup] = await q(
      `SELECT MAX(updated_at) AS at FROM trend_groups`
    );
    const groupAgeH = lastGroup?.at
      ? (Date.now() - new Date(lastGroup.at).getTime()) / 3600000
      : 999;

    if (groupAgeH >= 20) {
      await groupTrends().catch((e) =>
        console.warn('[групування] не вдалось:', e.message));

      // Широкі теми розкриваємо на конкретні речі, які під ними шукають.
      await expandBroad().catch((e) =>
        console.warn('[розкриття] не вдалось:', e.message));
    }

    // Ціни конкурентів збираємо завжди — вони не залежать від 1688.
    const top = await q(
      `SELECT g.id, g.name FROM trend_groups g
       JOIN trend_group_scores s ON s.group_id = g.id
       WHERE g.broad = false
    ...
    await refreshMarket(top).catch((e) =>
      console.warn('[ринок] не вдався:', e.message));

    // Товари підбираємо лише для нових груп і тих, де дані застаріли.
